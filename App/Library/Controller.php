...
abstract class Controller
{
	...
	protected function validate( $request, $validations )
	{
		...
		foreach ( $request as $key ) 
		{
			...
			if( $this->Auth()->check() )
			{
				...
				{
				// retornamos un mensaje de error al usuario
					array_push( $this->errors, "No tienes permisos para realizar esta acción. Por favor comunicarse con el administrador por la seguridad de la aplicacion." );
					break;
				}
			}
			...
			if( in_array( key($validations), $keys_request ) )
			{
				...
				foreach( $types as $type )
				{
					...
					if( $type == 'required' )
					{
						...
						if( is_null( $request[ key($validations) ] ) or empty( $request[ key($validations) ] )  or !isset( $request[ key($validations) ] ) )
							array_push( $this->errors, "El campo ". $this->validate_input_output( key($validations) ) ." es requerido" );
					}
					...
					if( $type == 'number' )
					{
						...
						if( !is_numeric( $request[ key($validations) ] ) )
							array_push( $this->errors, "El campo ". $this->validate_input_output( key($validations) ) ." no es un número" );
					}
					// validacion para emails
					if( $type == 'email' )
					{
						if ( !ValidateEmailTrait::comprobar_email( $request[ key($validations) ] ) ) {
							array_push( $this->errors, "El campo ". $this->validate_input_output( key($validations) ) ." no es una dirección de e-mail valida" );
						}
					}
					...
					if( $type[0] == 'unique' ){
						...
						if( !$validateunique->ValidateUnique( $request[ key($validations) ], key($validations), $type[1], $type[2] ) )
							// retornamos un mensaje de error al usuario
							array_push( $this->errors, "El " . $this->validate_input_output( key($validations) ) . " ". $request[ key($validations) ] ." ya existe." );
					}
				}
			}
			...
			echo '
				<div class="container my-4">
					<div class="row justify-content-center">
						<div class="col-8 alert alert-danger pb-0" >
							<ul class="pb-0">
			';
	...
	// validar el nombre del campo de salida para eliminar el "_" y la palabra "id"
	private function validate_input_output( $input )
	{
		// explotamos la raya piso para obtener un array
		$input_exp = explode( "_", $input );
		$input = '';
		// recorremos el array
		foreach ($input_exp as $word) 
		{
			if( $word != "id" )
				$input .= $word." ";
		}
