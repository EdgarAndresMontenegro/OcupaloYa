		...
		if ( isset($_GET['url']) ) {
			...
			for( $i = 0; $i < count($url); $i++ )
			{
				...
				if( isset( $url[$i] ) )
				{
					...
					{
						// recorremos todos las posiciones que tiene la url iniciando la $j en la posicion donde se encontro la carpeta con la variable $i
						for( $j = $i; $j < count($url); $j++ )
						{
							// validamos si la posición actual es una carpeta
							if( file_exists( '../App/Controllers/' . $this->folderCurrent . '/' . ucwords( $url[$j] ) ) )
							{
								// concotenamos el folder por si es mas de uno
								$this->folderCurrent .= ucwords($url[$j]) . '/';
								// agregamos esta posicion al array para ser eliminado
								array_push($url_delete, $j);
								// vamos cambiando el valor de la variable $i con la posición de la variable $j
								$i = $j;
							}
						}
						...
						else
						{
							...
							if( $i == (count($url)-1) )
							{
								// validamos si existe el controlador welcome en el interior de la carpeta y que no tenga nada más en la ruta
								if( !isset( $url[$i+1] ) && file_exists( '../App/Controllers/' . $this->folderCurrent . '/WelcomeController.php' ) )
								{
									// asignamos este valor al controlador
									$this->controllerCurrent = 'Welcome';
									// asignamos este metodo
									$this->methodCurrent = 'index';
								}
								else
